...
def send_text(message):
   try:
    ...
    if message.text == "🧾 VERIFIKASI":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_Silahkan Ketik YES untuk mengisi data._",
...
def send_text(message):
   try:
    ...
    if message.text == "🧾 VERIFIKASI":
        ...
        keyboard = telebot.types.ReplyKeyboardMarkup(True)
        keyboard.row('🚫 Cancel')
        send = bot.send_message(message.chat.id, "_⚠️Silahkan Ketik  YES untuk mengisi Data._",
                                parse_mode="Markdown", reply_markup=keyboard)
        # Next message will call the name_handler function
        bot.register_next_step_handler(message, trx_address)
    if message.text == "🎁 Bonus":
